...
exports.pack = function pack() {
    ...
    logger.info('>> process.env.NODE_ENV ', { env: process.env.NODE_ENV, mode, devtool });
    ...
    Object.keys(languages).map(function (language) {
        ...
        return webpack({
            ...
            if (jsonStats.errors.length > 0) {
                logger.warn('pack errors %s', jsonStats.errors.toString());
            }
            logger.info('>> Doc capture package generated for ' + language);
            if (jsonStats.warnings.length > 0) {
                logger.warn('pack warnings %s', jsonStats.warnings.toString());
            }
        });
    });
};
